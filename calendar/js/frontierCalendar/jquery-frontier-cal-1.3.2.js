/*
	...
	function calendar() 中添加了对已过期日的css归类
*/
	...
	function Calendar() {
		...
		*/
		...
		this.do_init = function(){
			...
			//自加。查找过期日，true为过期。
			function showOutdatedDateStyle(today, currentYearNum, currentMonthNum, currentDayNum, dayNum) {
				if(today.getFullYear() > currentYearNum) {
					return true;
				} else if(today.getFullYear() < currentYearNum) {
					return false;
				} else {
					if(today.getMonth() > currentMonthNum) {
						return true;
					} else if(today.getMonth() < currentMonthNum) {
						return false;
					} else {
			...
			for(var dayIndex = 0; dayIndex < Calendar.dayNames.length; dayIndex++){
				...
				if(dayIndex < firstDayWkIndex){
					...
					//自加
					if(showOutdatedDateStyle(today, currentYearNum, currentMonthNum, currentDayNum, dayNum)) {
						//calDayCell.removeClass();
						calDayCell.addClass("JFrontierCal-OutdatedDate-Day-Cell");
					} 
				...
				if(dayIndex == 6){
					...
					if(showOutdatedDateStyle(today, currentYearNum, currentMonthNum, currentDayNum, dayNum)){
						//calDayCell.removeClass();
						calDayCell.addClass("JFrontierCal-OutdatedDate-Day-Cell-Last");
		...
		this.renderSingleAgendaItem = function(agi){
			...
			if((DateUtil.daysDifference(firstRenderDate,lastRenderDate) + firstDtIndex) > 6){
				...
				if(agi.isAllDay()){
					...
					displayMessage = agi.getAgendaData("editor") + "&nbsp;&nbsp;&nbsp;" + agi.getTitle();  //根据需求修改
				...
				if(agi.isAllDay()){
					...
					displayMessage = agi.getAgendaData("editor") + "&nbsp;&nbsp;&nbsp;" + agi.getTitle();
					//displayMessage = DateUtil.getAgendaDisplayTime(agi.getStartDate())+" " + agi.getTitle();
				}
				isBegining = ((DateUtil.daysDifferenceDirection(agendaStartDate,firstRenderDate) == 0) ? true : false);
				isEnd = ((DateUtil.daysDifferenceDirection(agendaEndDate,lastRenderDate) == 0) ? true : false);
	...
	var jFrontierCalPlugin = function(
		...
		this.showMonth = function(calId,year,month){
			if(calId != null && year != null && month != null && year.length == 4 && (month.length == 1 || month.length == 2)){
				calId = stripNumberSign(calId);
				// strip any preceeding 0's
				//month = month.replace(/^[0]+/g,"");  //此为源代码，但忽略了1月即代号为0的情况。这么写会导致1月的代号0被削成空。
				month = month != 0 ? month.replace(/^[0]+/g,"") : 0;  //自加
